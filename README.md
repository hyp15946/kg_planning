...
기획서의 단계 목록에서 파트별 **개발 볼륨**을 산출한다.
`[키글] 앱 종합 스케쥴` 의 볼륨 칸에 넣을 값이 최종 산출물이다.
(디자인 볼륨 산출은 미완성이라 화면에서 숨겨 두었다 — `lib/config.ts` 의 `FEATURE_DESIGN`. 추후 추가 예정)
...
6. **3. 산출 결과** — 합계와 경고를 먼저 본다. 판단 필요 단계가 남아 있으면 여기서 알려 준다
7. **4. 개발 볼륨** — 기본으로 「판단 필요」 단계만 보인다. 등급을 고르고 확정 (체크 해제 시 전체 단계) · **5. 부대 항목** 필요 시 가산
8. 산출 결과를 스케쥴 문서에 옮기고, 필요하면 JSON/CSV로 내보낸다
